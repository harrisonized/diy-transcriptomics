## Unlike most of the other scripts, I wrote this one myself. 
...
# args
option_list = list(
    make_option(c("-i", "--input-file"), default="data/lemis/lemis_cleaned.tsv",
...
# ----------------------------------------------------------------------
# Most common live mammal

# 1. Identify the most common (by ‘quantity’) live mammal taken from the wild
...
# 2. Building on your analysis above, produce a plot showing live mammals
# (use ‘generic_name’) imported for the purposes of science/research.
# (tip: use geom_col() in ggplot for this). Feel free to play around with
# different themes to make your plot more exciting.
...
# ----------------------------------------------------------------------
# Countries with the most macaque imports

# 3. Identify the countries from which we import the most macaques
...
# 4. Using the same approach as above, create a plot showing the countries
# from which we import live bats.
...
# 6. How does the type of bat (use ‘specific_name’) imported differ between countries
...
# ----------------------------------------------------------------------
# Most expensive shipment

# 7. Identify the most expensive (by ‘value’) shipment of live mammals to enter the US.
log_print(paste(Sys.time(), 'Most expensive shipment of live mammals...'))
...
log_print(paste(Sys.time(), 'Most expensive shipment of any kind...'))
...
# 9. You are alerted to a concerning new viral disease of humans that is believed to originate from Fruit bats
...
# 10. A recent case of Anthrax in NYC was traced back to a contaminated Wildebeest hide that was stretched and
