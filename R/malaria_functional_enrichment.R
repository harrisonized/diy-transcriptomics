## Adapted from: Step7_functionalEnrichment
## Covers how to perform GSEA analysis using gProfiler2, clusterProfiler,
## CAMERA, ROAST, and GSVA.
## 
...
library('GSEABase')  # Gene Set Enrichment Analysis
...
# Start Log
start_time = Sys.time()
log <- log_open(paste0("malaria_functional_enrichment-",
...
top_hits <- topTable(ebFit, adjust ="BH", coef=1, number=50, sort.by="logFC")

# GO enrichment analysis
gost.res <- gost(rownames(top_hits), organism = "hsapiens", correction_method = "fdr")
...
gseaplot2(
    ...
    pvalue_table = FALSE,  # set this to FALSE for a cleaner plot
...
broadSet.C2.ALL <- getGmt(
    "/Users/danielbeiting/Dropbox/MSigDB/c2.all.v7.1.symbols.gmt",
...
GSVA.res.C2CP <- gsva(
    ...
    mx.diff=FALSE,
    method="gsva"  # Choose from: "gsva", ssgsea', "zscore" or "plage"
)
...
# the summary of the decideTests result shows how many sets were enriched in induced
# and repressed genes in all sample types
summary(res.C2CP)
...
# make a heatmap of differentially enriched gene sets using pearson correlation
hr.C2CP <- hclust(
    as.dist(1-cor(t(diffSets.C2CP), method="pearson")),
    method="complete"
)

# make a heatmap of differentially enriched gene sets using spearman correlation
hc.C2CP <- hclust(
    as.dist(1-cor(diffSets.C2CP, method="spearman")),
    method="complete"
)

# Cut the resulting tree and create color vector for clusters.  Vary the cut height 
# to give more or fewer clusters, or you the 'k' argument to force n number of clusters
...
# colorpanel(40, "darkblue", "yellow", "white");
# heat.colors(75); cm.colors(75); rainbow(75); redgreen(75);
# library(RColorBrewer); rev(brewer.pal(9,"Blues")[-1])
